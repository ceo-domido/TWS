...
public class Testbed {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		wrapper.collection = database.getCollection("historicalData");
        //! [ereader]
        Thread.sleep(1000);
		
		requestHistory(wrapper.getClient(), ContractSamples.USStockWithPrimaryExch(), "MIDPOINT");
        ...
        private static String getDuration(String barSize) {
            switch (barSize) {
                case "1 day":
                    return "1 Y";
                case "1 hour":
        ...
        private static void requestHistory(EClientSocket client, Contract contract, String dataType ) throws InterruptedException {
            Calendar startOfPeriod = Calendar.getInstance(TimeZone.getTimeZone("EST"));
            startOfPeriod.add(Calendar.DAY_OF_YEAR, -1);
			MongoCollection collection = ((EWrapperImpl) client.wrapper()).collection;
			collection.deleteMany(gte("timestamp", startOfPeriod.getTimeInMillis()));
			...
			endOfPeriod.add(Calendar.MINUTE, -1);
			Document lastDoc;
			do  {
				requestHistoryBatch(client,item,startOfPeriod);
				lastDoc = (Document) collection.find(gte("timestamp", startOfPeriod.getTimeInMillis()))
						.sort(Sorts.descending("timestamp")).first();
				if (lastDoc == null) break;
				startOfPeriod.setTimeInMillis(lastDoc.getLong("timestamp"));
				startOfPeriod.add(Calendar.MINUTE, duration);
				while (!((EWrapperImpl) client.wrapper()).requests.isEmpty()) {
					Thread.sleep(1000);
				}
			} while (lastDoc.getLong("timestamp") <= endOfPeriod.getTimeInMillis());
        }

        private static void requestHistoryBatch(EClientSocket client, HistoryItem item, Calendar startDate) {
			SimpleDateFormat form = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
			String formatted = form.format(startDate.getTime());
			int reqId = new Random().nextInt();
			((EWrapperImpl) client.wrapper()).requests.put(reqId, item);
