...
public class EWrapperImpl implements EWrapper {
	...
	public void historicalData(int reqId, Bar bar) {
			...
			try {
				parsingDate.setTime(ft.parse(bar.time()));
				Document doc = new Document();
				doc.append("_id", new StringBuilder()
						.append(item.getContractId())
						.append("-")
						.append(parsingDate.getTimeInMillis()).toString());
				doc.append("barSize", item.barSize);
				doc.append("dataType", item.dataType);
				doc.append("exchange", item.contract.exchange());
				doc.append("secType", item.contract.getSecIdType());
				doc.append("currency", item.contract.currency());
				doc.append("symbol", item.contract.symbol());
				doc.append("timestamp", parsingDate.getTimeInMillis());
				doc.append("time", bar.time());
				doc.append("open", bar.open());
				doc.append("high", bar.high());
				doc.append("low", bar.close());
				doc.append("close",bar.close());
				doc.append("volume", bar.volume());
				doc.append("count", bar.count());
				doc.append("wap",bar.wap());
				collection.insertOne(doc);
//				history.get(requests.get(reqId)).put(parsingDate, bar);
			} catch (ParseException e) {
			}
            System.out.println("HistoricalData. "+reqId+" - Date: "+bar.time());
	}
	//! [historicaldata]
